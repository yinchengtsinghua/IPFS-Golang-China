...
{ //TODO:在“ipfs add”中使用coreapi add后，请考虑将此默认值设为inline
			name: "addInlineRaw",
			data: strFile(helloStr),
...
buf := make([]byte, 1) //非零，以便read（）实际尝试读取
...
//todo（lgierth）应该正确测试，len（links）>0
