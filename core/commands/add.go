...
//errDepthLimitExceeded表示已超过最大深度。
...
cmds.OptionRecursivePath, //允许递归路径的内置选项（-r，-recursive）
cmds.OptionDerefArgs,     //一个内置选项，用于解析传入的文件系统链接（--dereference args）
...
//除非使用quiet或silent，否则ipfs cli进度条默认为true。
...
//不要在这里设置或打印错误，这在下面的goroutine中发生
						return
					}
				}
